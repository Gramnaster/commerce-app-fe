  ...
  return (
    <div className="h-full">
      <div className="h-[468px] border-r border-[#808080] ">
        ...
        <div className='font-bold place-items-end'>
          <div>Categories</div>
          <button onClick={() => setSelectedCategory(null)} className='m-1 rounded-2xl hover:cursor-pointer hover:bg-[#BE493D] hover:text-white'>All</button>
          {categoryData.map((category: ProductCategory) => {
            const { id, title } = category;
            return (
              <button onClick={() => setSelectedCategory(`${title}`)} className='m-1 rounded-2xl hover:cursor-pointer hover:bg-[#BE493D] hover:text-white' key={id}>{title}</button>
            )
          })}
        </div>
      </div>
