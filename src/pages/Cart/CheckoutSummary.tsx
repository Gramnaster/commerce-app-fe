...
interface CheckoutSummaryProps {
  addressId: number | null; // Changed from userAddressId to addressId
...
const CheckoutSummary = ({
  addressId, // Changed from userAddressId
  onOrderComplete,
  SocialPrograms,
  selectedSP
  ...
  const handleCompletePayment = async () => {
    ...
    try {
      // Create user cart order - now using address_id instead of user_address_id
      await customFetch.post('/user_cart_orders', {
        user_cart_order: {
          address_id: addressId, // Changed from user_address_id
  ...
  return (
    <div className="bg-base-100 border-l border-[#808080] rounded-none p-6 sticky">
      ...
      {loading ? (
        ...
        <div className={`mt-4 rounded-lg max-h-[40px] ${!hasEnoughBalance ? 'max-h-[60px]' : ''}`}>
      ...
      <button
        ...
        disabled={!addressId || !hasEnoughBalance || processingPayment || loading}
      >
        {processingPayment ? (
          <span className="loading loading-spinner loading-sm"></span>
        ) : (
